...
class SizeCheckWrapper(object):
    ...
    def read_line_by_boundary(self, next, last, size):
        data = self.rfile.read_line_by_boundary(next, last, size)
        self.bytes_read += len(data)
        self._check_length()
        return data
        pass
...
if not _fileobject_uses_str_type:
    ...
    class CP_fileobject(socket._fileobject):
        ...
        def read_line_by_boundary(self, next_boundary, last_boundary, size):
            data = self._rbuf
            # Read until size bytes or \n or EOF seen, whichever comes first
            boundarys = [ '\n', last_boundary, next_boundary ]  ## must be the order
            for boundary in boundarys:
                nl = data.find(boundary, 0, size)
                if nl >= 0:
                    nl += len(boundary) 
                    self._rbuf = data[nl:]
                    return data[:nl]
